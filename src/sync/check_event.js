...
export default async function(groupId){
  try{
    ...
    await gaCache.set('settings', { _id: 'current_event' }, gaEvent)
    ...
    if(!gaEvent?.notified) log.info(`missing ${missing?.length || 0 }/${lastBracketId + 1} brackets for ${groupId}`)
    ...
    if(!gaEvent?.notified){
      await gaCache.set('gaEventList', { _id: groupId }, { notified: true })
